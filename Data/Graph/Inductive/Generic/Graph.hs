...
instance (Show n, Show a) => Show (LPath n a) where
  show (LP xs) = show xs
...
-- of the 'Graph'.
type Decomp gr a b = (MContext (Node gr) a b,gr a b)
-- | The same as 'Decomp', only more sure of itself.
type GDecomp gr a b  = (Context (Node gr) a b,gr a b)

-- | Unlabeled context.
type UContext n    = ([n],n,[n])
-- | Unlabeled decomposition.
type UDecomp gr    = (Maybe (UContext (Node gr)),gr () ())

--type family GraphOver n :: * -> *
-- | Minimum implementation: 'empty', 'isEmpty', 'match', 'mkGraph', 'labNodes'
class (Ord (Node gr)) => Graph gr where
  ...
  --   'empty'@.
  mkGraph   :: [LNode (Node gr) a] -> [LEdge (Node gr) b] -> gr a b

  -- | A list of all 'LNode's in the 'Graph'.
  labNodes  :: gr a b -> [LNode (Node gr) a]
  ...
  labEdges  :: gr a b -> [LEdge (Node gr) b]
...
class (Graph gr) => DynGraph gr where
  ...
  (&) :: Context (Node gr) a b -> gr a b -> gr a b

-- | Fold a function over the graph.
ufold :: (Graph gr) => ((Context (Node gr) a b) -> c -> c) -> c -> gr a b -> c
ufold f u g
  | isEmpty g = u
...
gmap :: (DynGraph gr)
     => (Context (Node gr) a b -> Context (Node gr) c d)
     -> gr a b -> gr c d
...
nodes :: (Graph gr) => gr a b -> [Node gr]
nodes = map fst . labNodes

-- | List all 'Edge's in the 'Graph'.
edges :: (Graph gr) => gr a b -> [Edge (Node gr)]
...
toLEdge :: Edge n -> b -> LEdge n b
toLEdge (v,w) l = (v,w,l)

-- | The label in an edge.
edgeLabel :: LEdge n b -> b
edgeLabel (_,_,l) = l

---- | List N available 'Node's, i.e. 'Node's that are not used in the 'Graph'.
--newNodes :: (Graph gr) => Int -> gr a b -> [Node gr]
--newNodes i g
--  | isEmpty g = [0..i-1]
--  | otherwise = [n+1..n+i]
--  where
--    (_,n) = nodeRange g
-- | List N available 'Node's, starting from n0, i.e. 'Node's that are not used
-- in the 'Graph'. Unlike previous implementation of 'newNodes', 'newNodesFrom'
-- will use unallocated nodes in the range '(n0,maxNode)'.
newNodesFrom :: (Graph gr,Enum (Node gr)) => Int -> Node gr -> gr a b -> [Node gr]
newNodesFrom i n0 g = take i ([n0..] \\ nodes g)

-- | 'True' if the 'Node' is present in the 'Graph'.
gelem :: (Graph gr) => Node gr -> gr a b -> Bool
gelem v = isJust . fst . match v

-- | Insert a 'LNode' into the 'Graph'.
insNode :: (DynGraph gr) => LNode (Node gr) a -> gr a b -> gr a b
insNode (v,l) = (([],v,l,[])&)
{-# NOINLINE [0] insNode #-}

-- | Insert a 'LEdge' into the 'Graph'.
insEdge :: (DynGraph gr) => LEdge (Node gr) b -> gr a b -> gr a b
insEdge (v,w,l) g = (pr,v,la,(l,w):su) & g'
  where
    (Just (pr,_,la,su),g') = match v g

-- | Remove a 'Node' from the 'Graph'.
delNode :: (Graph gr) => Node gr -> gr a b -> gr a b
...
--   'delLEdge'.
delEdge :: (DynGraph gr) => Edge (Node gr) -> gr a b -> gr a b
...
buildGr :: (DynGraph gr) => [Context (Node gr) a b] -> gr a b
buildGr = foldr (&) empty

-- | Build a quasi-unlabeled 'Graph'.
mkUGraph :: (Graph gr) => [(Node gr)] -> [Edge (Node gr)] -> gr () ()
...
slabNodes :: (Eq a,Graph gr) => gr a b -> [LNode (Node gr) a]
slabNodes = sortBy (compare `on` fst) . labNodes

glabEdges :: (Eq b, Graph gr) => gr a b -> [GroupEdges (Node gr) b]
...
newtype GroupEdges n b = GEs (LEdge n [b])

--deriving instance (Show (n), Show b) => Show (GroupEdges gr b)

instance (Eq b,Eq n) => Eq (GroupEdges n b) where
...
--
context1l :: (Graph gr) => gr a b -> Node gr -> Adj (Node gr) b
context1l = maybe [] context1l' .: mcontext

context4l :: (Graph gr) => gr a b -> Node gr -> Adj (Node gr) b
context4l = maybe [] context4l' .: mcontext

mcontext :: (Graph gr) => gr a b -> Node gr -> MContext (Node gr) a b
mcontext = fst .: flip match

context1l' :: Eq n => Context n a b -> Adj n b
context1l' (p,v,_,s) = p++filter ((==v).snd) s

context4l' :: Eq n => Context n a b -> Adj n b
...
prettify :: (DynGraph gr, Show a, Show b, Show (Node gr)) => gr a b -> String
...
prettyPrint :: (DynGraph gr, Show a, Show b, Show (Node gr)) => gr a b -> IO ()
