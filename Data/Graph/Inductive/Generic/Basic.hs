...
gsel :: (Graph gr) => (Context (Node gr) a b -> Bool) -> gr a b -> [Context (Node gr) a b]
...
efilter :: (DynGraph gr) => (LEdge (Node gr)b -> Bool) -> gr a b -> gr a b
...
threadGraph :: (Graph gr) => (Context (Node gr) a b -> r -> t)
               -> Split (gr a b) (Context (Node gr) a b) r -> SplitM (gr a b) (Node gr) t
threadGraph f c = threadMaybe f c match

-- gfold1 f d b u = threadGraph (\c->d (labNode' c)) (\c->gfoldn f d b u (f c))
gfold1 :: (Graph gr) => (Context (Node gr) a b -> [Node gr]) -> (Context (Node gr) a b -> r -> t)
          -> Collect (Maybe t) r -> SplitM (gr a b) (Node gr) t
gfold1 f d b = threadGraph d (\c->gfoldn f d b (f c))

gfoldn :: (Graph gr) => (Context (Node gr) a b -> [Node gr]) -> (Context (Node gr) a b -> r -> t)
          -> Collect (Maybe t) r -> [Node gr] -> gr a b -> (r, gr a b)
gfoldn f d b = threadList b (gfold1 f d b)

-- gfold :: ((Context (Node gr) a b) -> [Node gr]) -> ((Node gr,a) -> c -> d) ->
--          (Maybe d -> c -> c) -> c -> [Node gr] -> Graph a b -> c
-- gfold f d b u l g = fst (gfoldn f d b u l g)

-- type Dir a b    = (Context (Node gr) a b) -> [Node gr]  -- direction of fold
-- type Dagg a b c = (Node gr,a) -> b -> c       -- depth aggregation
-- type Bagg a b   = (Maybe a -> b -> b,b)    -- breadth/level aggregation
--
-- gfold :: (Dir a b) -> (Dagg a c d) -> (Bagg d c) -> [Node gr] -> Graph a b -> c
-- gfold f d (b,u) l g = fst (gfoldn f d b u l g)

-- | Directed graph fold.
gfold :: (Graph gr) =>   ((Context (Node gr) a b) -> [Node gr])    -- ^ direction of fold
        -> ((Context (Node gr) a b) -> c -> d)    -- ^ depth aggregation
