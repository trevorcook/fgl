...
iDom :: (Graph gr) => gr a b -> Node gr -> [(Node gr,Node gr)]
...
dom :: (Graph gr) => gr a b -> Node gr -> [(Node gr,[Node gr])]
dom g root = let
...
idomWork :: (Graph gr) => gr a b -> Node gr -> (IDom, ToNode (Node gr), FromNode (Node gr))
idomWork g root = let
    ...
    preds = array (1, s-1) [(i, filter (/= -1) (map (fromNode M.!)
...
getDom :: ToNode n-> IDom -> Array Node' [n]
...
{-
:m +Data.Graph.Inductive.Generic
