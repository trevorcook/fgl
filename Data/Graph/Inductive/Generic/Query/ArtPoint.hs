...
------------------------------------------------------------------------------
data DFSTree v a = B (v,a,[(v,a)]) [DFSTree v a]
...
------------------------------------------------------------------------------
data LOWTree v a = Brc (v,a,a) [LOWTree v a]
     deriving (Eq, Show, Read)

------------------------------------------------------------------------------
-- Finds the back edges for a given node.
------------------------------------------------------------------------------
getBackEdges :: Eq n => n -> [[(n,Int)]] -> [(n,Int)]
getBackEdges _ [] = []
...
------------------------------------------------------------------------------
-- Original implementation, depends on Num (Node gr)
--dfsTree :: (Graph gr) => Int -> Node gr -> [Node gr] -> [[(Node gr,Int)]] ->
--                       gr a b -> ([DFSTree (Node gr) Int],gr a b,Int)
--dfsTree n _ []      _ g             = ([],g,n)
...
--                                    sc'        = map (\x->(x,0)) sc
--                                    sc         = suc' c
-- Reimplementation. u isn't called except on a recursive call, so does
-- it even need to be used in the first place? I take it out.
dfsTree :: (Graph gr) => Int -> [Node gr] -> [[(Node gr,Int)]] ->
                       gr a b -> ([DFSTree (Node gr) Int],gr a b,Int)
dfsTree n []      _ g             = ([],g,n)
dfsTree n _       _ g | isEmpty g = ([],g,n)
dfsTree n (v:vs) ls g = case match v g of
                            (Nothing, g1) -> dfsTree n vs ls g1
                            (Just c , g1) -> (B (v,n+1,bck) ts:ts', g3, k)
                             where  bck        = getBackEdges v ls
                                    (ts, g2,m) = dfsTree (n+1) sc ls' g1
                                    (ts',g3,k) = dfsTree m vs ls g2
                                    ls'        = ((v,n+1):sc'):ls
                                    sc'        = map (\x->(x,0)) sc
                                    sc         = suc' c
...
------------------------------------------------------------------------------
lowTree :: DFSTree n Int -> LOWTree n Int
...
-- contains: the node number v, the DFS number n, and the low number low.
------------------------------------------------------------------------------
getLowTree :: (Graph gr) => gr a b -> Node gr -> LOWTree (Node gr) Int
getLowTree g v = lowTree (head dfsf)
                  where (dfsf, _, _) = dfsTree 0 [v] [] g
...
------------------------------------------------------------------------------
artpoints :: (Graph gr) => gr a b -> Node gr -> [Node gr]
