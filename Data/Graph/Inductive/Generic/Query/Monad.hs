...
--
getNode :: (GraphM m gr) => GT m (gr a b) (Node gr)
getNode = MGT (\mg->do {((_,v,_,_),g) <- matchAnyM mg; return (v,g)})

getContext :: (GraphM m gr) => GT m (gr a b) (Context (Node gr) a b)
...
--
getNodes' :: (Graph gr,GraphM m gr) => GT m (gr a b) [Node gr]
getNodes' = condMGT' isEmpty (return [])
                             (do v  <- getNode
                                 vs <- getNodes
                                 return (v:vs))

getNodes :: (GraphM m gr) => GT m (gr a b) [Node gr]
getNodes = condMGT isEmptyM (return [])
                            (do v  <- getNode
                                vs <- getNodes
                                return (v:vs))

sucGT :: (GraphM m gr) => Node gr -> GT m (gr a b) (Maybe [Node gr])
...
sucM :: (GraphM m gr) => Node gr -> m (gr a b) -> m (Maybe [Node gr])
...
graphUFold :: (GraphM m gr) => (Context (Node gr) a b -> c -> c) -> c -> GT m (gr a b) c
...
-- instances of graphRec
--
graphNodesM0 :: (GraphM m gr) => GT m (gr a b) [Node gr]
graphNodesM0 = graphRec getNode (:) []

graphNodesM :: (GraphM m gr) => GT m (gr a b) [Node gr]
graphNodesM = graphUFold (\(_,v,_,_)->(v:)) []

graphNodes :: (GraphM m gr) => m (gr a b) -> m [Node gr]
graphNodes = runGT graphNodesM

graphFilterM :: (GraphM m gr) => (Context (Node gr) a b -> Bool) ->
                              GT m (gr a b) [Context (Node gr) a b]
graphFilterM p = graphUFold (\c cs->if p c then c:cs else cs) []

graphFilter :: (GraphM m gr) => (Context (Node gr) a b -> Bool) -> m (gr a b) -> m [Context (Node gr) a b]
...
dfsGT :: (GraphM m gr) => [Node gr] -> GT m (gr a b) [Node gr]
dfsGT []     = return []
dfsGT (v:vs) = MGT (\mg->
               do (mc,g') <- matchM v mg
                  case mc of
...
dfsM :: (GraphM m gr) => [Node gr] -> m (gr a b) -> m [Node gr]
dfsM vs = runGT (dfsGT vs)

dfsM' :: (GraphM m gr) => m (gr a b) -> m [Node gr]
dfsM' mg = do {vs <- nodesM mg; runGT (dfsGT vs) mg}


-- | depth-first search yielding dfs forest
dffM :: (GraphM m gr) => [Node gr] -> GT m (gr a b) [Tree (Node gr)]
dffM vs = MGT (\mg->
          do g<-mg
             b<-isEmptyM mg
...
graphDff :: (GraphM m gr) => [Node gr] -> m (gr a b) -> m [Tree (Node gr)]
graphDff vs = runGT (dffM vs)

graphDff' :: (GraphM m gr) => m (gr a b) -> m [Tree (Node gr)]
