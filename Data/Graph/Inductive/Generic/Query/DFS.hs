...
--
fixNodes :: (Graph gr) => ([Node gr] -> gr a b -> c) -> gr a b -> c
...
--   result of xdffWith)
--
type CFun n a b c = Context n a b -> c

xdfsWith :: (Graph gr)
         => CFun (Node gr) a b [Node gr] -> CFun (Node gr) a b c -> [Node gr]
         -> gr a b -> [c]
xdfsWith _ _ []     _             = []
...
-- dfs
--
dfsWith :: (Graph gr) => CFun (Node gr) a b c -> [Node gr] -> gr a b -> [c]
dfsWith = xdfsWith suc'

dfsWith' :: (Graph gr) => CFun (Node gr) a b c -> gr a b -> [c]
dfsWith' f = fixNodes (dfsWith f)

dfs :: (Graph gr) => [Node gr] -> gr a b -> [Node gr]
dfs = dfsWith node'

dfs' :: (Graph gr) => gr a b -> [Node gr]
dfs' = dfsWith' node'


-- undirected dfs, ie, ignore edge directions
--
udfs :: (Graph gr) => [Node gr] -> gr a b -> [Node gr]
udfs = xdfsWith neighbors' node'

udfs' :: (Graph gr) => gr a b -> [Node gr]
udfs' = fixNodes udfs


-- reverse dfs, ie, follow predecessors
--
rdfs :: (Graph gr) => [Node gr] -> gr a b -> [Node gr]
rdfs = xdfsWith pre' node'

rdfs' :: (Graph gr) => gr a b -> [Node gr]
rdfs' = fixNodes rdfs


-- generalized depth-first forest
--
xdfWith :: (Graph gr) => CFun (Node gr) a b [Node gr] -> CFun (Node gr) a b c -> [Node gr] -> gr a b -> ([Tree c],gr a b)
xdfWith _ _ []     g             = ([],g)
...
xdffWith :: (Graph gr) => CFun (Node gr) a b [Node gr] -> CFun (Node gr) a b c -> [Node gr] -> gr a b -> [Tree c]
xdffWith d f vs g = fst (xdfWith d f vs g)


-- dff
--
dffWith :: (Graph gr) => CFun (Node gr) a b c -> [Node gr] -> gr a b -> [Tree c]
dffWith = xdffWith suc'

dffWith' :: (Graph gr) => CFun (Node gr) a b c -> gr a b -> [Tree c]
dffWith' f = fixNodes (dffWith f)

dff :: (Graph gr) => [Node gr] -> gr a b -> [Tree (Node gr)]
dff = dffWith node'

dff' :: (Graph gr) => gr a b -> [Tree (Node gr)]
dff' = dffWith' node'


-- undirected dff
--
udffWith :: (Graph gr) => CFun (Node gr) a b c -> [Node gr] -> gr a b -> [Tree c]
udffWith = xdffWith neighbors'

udffWith' :: (Graph gr) => CFun (Node gr) a b c -> gr a b -> [Tree c]
udffWith' f = fixNodes (udffWith f)

udff :: (Graph gr) => [Node gr] -> gr a b -> [Tree (Node gr)]
udff = udffWith node'

udff' :: (Graph gr) => gr a b -> [Tree (Node gr)]
udff' = udffWith' node'


-- reverse dff, ie, following predecessors
--
rdffWith :: (Graph gr) => CFun (Node gr) a b c -> [Node gr] -> gr a b -> [Tree c]
rdffWith = xdffWith pre'

rdffWith' :: (Graph gr) => CFun (Node gr) a b c -> gr a b -> [Tree c]
rdffWith' f = fixNodes (rdffWith f)

rdff :: (Graph gr) => [Node gr] -> gr a b -> [Tree (Node gr)]
rdff = rdffWith node'

rdff' :: (Graph gr) => gr a b -> [Tree (Node gr)]
rdff' = rdffWith' node'
...
components :: (Graph gr) => gr a b -> [[Node gr]]
