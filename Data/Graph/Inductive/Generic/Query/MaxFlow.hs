...
--          j
-- @
getRevEdges :: (Num b,Ord b,Eq n) => [(n,n)] -> [(n,n,b)]
getRevEdges [] = []
...
updAdjList::(Num b,Ord b,Eq n) => [((b,b,b),n)]->n->b->Bool->[((b,b,b),n)]
...
updateFlow :: (DynGraph gr,Num b,Ord b) => Path (Node gr) -> b -> gr a (b,b,b) -> gr a (b,b,b)
updateFlow []       _ g = g
updateFlow [_]       _ g = g
...
mfmg :: (DynGraph gr,Num b,Ord b) => gr a (b,b,b) -> Node gr -> Node gr -> gr a (b,b,b)
...
-- (x,y,z)=(max capacity,current flow,residual capacity) and all edges
-- are of the form a\<---->b
mf :: (DynGraph gr,Num b,Ord b) => gr a b -> Node gr -> Node gr -> gr a (b,b,b)
...
maxFlowgraph :: (DynGraph gr,Num b,Ord b) => gr a b -> Node gr -> Node gr -> gr a (b,b)
maxFlowgraph g s t = emap (\(u,v,_)->(v,u)) g2
                           where g2 = elfilter (\(x,_,_)->x/=0) g1
                                 g1 = mf g s t

-- | Compute the value of a maximumflow
maxFlow :: (DynGraph gr,Num b,Ord b) => gr a b -> Node gr -> Node gr -> b
